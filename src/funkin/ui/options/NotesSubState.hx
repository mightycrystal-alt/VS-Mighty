...
class NotesSubState extends MusicBeatSubstate
{
	...
	public function new() {
		...
		for (i in 0...Preferences.data.arrowHSV.length) {
			var yPos:Float = (165 * i) + 35;
			for (j in 0...3) {
				var optionText:Alphabet = new Alphabet(posX + (225 * j) + 250, yPos + 60, Std.string(Preferences.data.arrowHSV[i][j]), true);
			...
			note.antialiasing = Preferences.data.globalAntialiasing;
			grpNotes.add(note);

			var newShader:ColorSwap = new ColorSwap();
			note.shader = newShader.shader;
			newShader.hue = Preferences.data.arrowHSV[i][0] / 360;
			newShader.saturation = Preferences.data.arrowHSV[i][1] / 100;
			newShader.brightness = Preferences.data.arrowHSV[i][2] / 100;
	...
	function changeSelection(change:Int = 0) {
		curSelected += change;
		if (curSelected < 0)
			curSelected = Preferences.data.arrowHSV.length-1;
		if (curSelected >= Preferences.data.arrowHSV.length)
			curSelected = 0;

		curValue = Preferences.data.arrowHSV[curSelected][typeSelected];
		updateValue();
	...
	function resetValue(selected:Int, type:Int) {
		curValue = 0;
		Preferences.data.arrowHSV[selected][type] = 0;
		switch(type) {
	...
	function updateValue(change:Float = 0) {
		...
		Preferences.data.arrowHSV[curSelected][typeSelected] = roundedValue;
