...
import funkin.ui.debug.EditorPlayState;
...
import funkin.play.Song.SwagSong;
import flixel.FlxG;
...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	override function create()
	{
		...
		else
		{
			...
			_song = {
				...
				stage: 'stage',
				validScore: false,
				tripleStrumline: false
			};
			addSection();
			PlayState.SONG = _song;
		}
		tripleCharacter = _song.tripleStrumline == true;
		...
		strumLine = new FlxSprite(GRID_SIZE, 50).makeGraphic(Std.int(GRID_SIZE * getLaneCount()), 4);
		add(strumLine);
	...
	var tripleCharacter:Bool = false;

	inline function getLaneCount():Int
	{
		return tripleCharacter ? 12 : 8;
	}

	inline function getHalfLaneCount():Int
	{
		return Std.int(getLaneCount() / 2);
	}
	...
	function addSongUI():Void
	{
		...
		var tripleStrumline = new FlxUICheckBox(10, 40, null, null, "Triple Strumline", 100);
		tripleStrumline.checked = tripleCharacter;
		tripleStrumline.callback = function()
		{
			tripleCharacter = tripleStrumline.checked;
			_song.tripleStrumline = tripleCharacter;
			strumLine.makeGraphic(Std.int(GRID_SIZE * getLaneCount()), 4);
			for (i in 0...strumLineNotes.length)
			{
				strumLineNotes.members[i].visible = i < getLaneCount();
			}
			reloadGridLayer();
			updateThirdIcon();
		};
		...
		tab_group_song.add(tripleStrumline);
	...
	function addSectionUI():Void
	{
		...
		var swapSection:FlxButton = new FlxButton(10, check_notesSec.y + 40, "Swap section", function()
		{
			...
			{
				var note:Array<Dynamic> = _song.notes[curSec].sectionNotes[i];
				note[1] = (note[1] + getHalfLaneCount()) % getLaneCount();
				_song.notes[curSec].sectionNotes[i] = note;
			}
			updateGrid();
		});

		var dadToGFSection:FlxButton = new FlxButton(swapSection.x + 100, swapSection.y, "Dad to GF", function() 
{
    for (i in 0..._song.notes[curSec].sectionNotes.length)
    {
        var note:Array<Dynamic> = _song.notes[curSec].sectionNotes[i];
        
        // Check if the note is in the first lane (Dad: columns 0, 1, 2, 3)
        if (note[1] >= 0 && note[1] < 4)
        {
            note[1] += 8; // Shift to the third lane (GF: columns 8, 9, 10, 11)
        }
    ...
    updateGrid();
		...
		tab_group_section.add(dadToGFSection);
	...
	override function update(elapsed:Float)
	{
		...
		if (!blockInput)
		{
			...
			{
				autosaveSong();
				LoadingState.loadAndSwitchState(new funkin.ui.debug.EditorPlayState(sectionStartTime()));
			}
		...
		curRenderedNotes.forEachAlive(function(note:Note) {
			...
			if(curSelectedNote != null) {
				var noteDataToCheck:Int = note.noteData;
					if(noteDataToCheck > -1 && note.mustPress != _song.notes[curSec].mustHitSection) noteDataToCheck += getHalfLaneCount();
	...
	function reloadGridLayer() {
		gridLayer.clear();
		gridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE * getLaneCount(), Std.int(GRID_SIZE * getSectionBeats() * 4 * zoomList[curZoom]));
		gridBG.x = GRID_SIZE;
		...
		{
			nextGridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE * getLaneCount(), Std.int(GRID_SIZE * getSectionBeats(curSec + 1) * 4 * zoomList[curZoom]));
			nextGridBG.x = GRID_SIZE;
		...
		if(foundNextSec)
		{
			var gridBlack:FlxSprite = new FlxSprite(GRID_SIZE, gridBG.height).makeGraphic(Std.int(GRID_SIZE * getLaneCount()), Std.int(nextGridBG.height), FlxColor.BLACK);
			gridBlack.alpha = 0.4;
		...
		if (tripleCharacter)
		{
			var gridBlackLine:FlxSprite = new FlxSprite(gridBG.x + gridBG.width / 3).makeGraphic(2, leHeight, FlxColor.BLACK);
			gridLayer.add(gridBlackLine);
			var gridBlackLine2:FlxSprite = new FlxSprite(gridBG.x + gridBG.width * 2 / 3).makeGraphic(2, leHeight, FlxColor.BLACK);
			gridLayer.add(gridBlackLine2);
		}
		else
		{
			var gridBlackLine:FlxSprite = new FlxSprite(gridBG.x + gridBG.width / 2).makeGraphic(2, leHeight, FlxColor.BLACK);
			gridLayer.add(gridBlackLine);
		}
	...
	function updateWaveform() {
		#if desktop
		if(waveformPrinted) {
			waveformSprite.makeGraphic(Std.int(GRID_SIZE * getLaneCount()), Std.int(gridBG.height), 0x00FFFFFF);
		...
		// Draws
		var gSize:Int = Std.int(GRID_SIZE * getLaneCount());
	...
	function updateThirdIcon():Void
	{
		if (!tripleCharacter)
		{
			if (player3Icon != null)
			{
				remove(player3Icon, true);
				player3Icon = null;
			}
			return;
		}

		var thirdCharacter:String = _song.gfVersion;

		if (player3Icon == null)
		{
			player3Icon = new HealthIcon(loadHealthIconFromCharacter(thirdCharacter));
		...
		{
			player3Icon.changeIcon(loadHealthIconFromCharacter(thirdCharacter));
		}

		player3Icon.setPosition(GRID_SIZE + (GRID_SIZE * getLaneCount()) / 1.5, -100);
	}
	...
	function setupNoteData(i:Array<Dynamic>, isNextSection:Bool):Note
	{
		...
		if(isNextSection && _song.notes[curSec].mustHitSection != _song.notes[curSec+1].mustHitSection) {
			if(daNoteInfo >= getHalfLaneCount()) {
				note.x -= GRID_SIZE * getHalfLaneCount();
			} else if(daSus != null) {
				note.x += GRID_SIZE * getHalfLaneCount();
			}
		}
	...
	function selectNote(note:Note):Void
	{
		...
		{
			if(note.mustPress != _song.notes[curSec].mustHitSection) noteDataToCheck += getHalfLaneCount();
			for (i in _song.notes[curSec].sectionNotes)
			{
	...
	private function addNote(strum:Null<Float> = null, data:Null<Int> = null, type:Null<Int> = null):Void
	{
		...
		{
			_song.notes[curSec].sectionNotes.push([noteStrum, (noteData + getHalfLaneCount()) % getLaneCount(), noteSus, noteTypeIntMap.get(daType)]);
		}
