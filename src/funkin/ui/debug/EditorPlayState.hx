...
class EditorPlayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		strumLine = new FlxSprite(Preferences.data.middleScroll ? PlayState.STRUM_X_MIDDLESCROLL : PlayState.STRUM_X, 50).makeGraphic(FlxG.width, 10);
		if(Preferences.data.downScroll) strumLine.y = FlxG.height - 150;
	...
	function sayGo() {
		...
		go.screenCenter();
		go.antialiasing = Preferences.data.globalAntialiasing;
		add(go);
	...
	private function endSong() {
		LoadingState.loadAndSwitchState(new funkin.ui.debug.charting.ChartingState());
	}
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		rating.visible = !Preferences.data.hideHud;
		...
		comboSpr.visible = !Preferences.data.hideHud;
	...
	function spawnNoteSplash(x:Float, y:Float, data:Int, ?note:Note = null) {
		...
		var hue:Float = Preferences.data.arrowHSV[data % 4][0] / 360;
		var sat:Float = Preferences.data.arrowHSV[data % 4][1] / 100;
		var brt:Float = Preferences.data.arrowHSV[data % 4][2] / 100;
		if(note != null) {
