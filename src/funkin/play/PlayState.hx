...
import funkin.graphics.BackgroundDancer;
import funkin.graphics.BackgroundGirls;
...
import funkin.play.components.NoteSplash;
import funkin.graphics.shaders.PhillyGlowGradient;
import funkin.graphics.shaders.PhillyGlowParticle;
import funkin.ranked.RankedStats;
import funkin.play.components.Rating;
import funkin.ui.story.StoryMenuState;
import funkin.play.components.StrumNote;
import funkin.graphics.TankmenBG;
...
import funkin.play.components.Note.EventNote;
...
class PlayState extends MusicBeatState
{
	...
	public var comboRating:FlxText;
	var timeTxt:FlxText;
	...
	public var timeShown:Int = 0;
	public var currentTimingShown:FlxText = null;
	public var timingFadeTimer:Int = 0;
	...
	override public function create()
	{
		...
		debugKeysChart = Preferences.keyBinds.get('debug_1').copy();
		debugKeysCharacter = Preferences.keyBinds.get('debug_2').copy();
		PauseSubState.songName = null; // Reset to default
		playbackRate = Preferences.getGameplaySetting('songspeed', 1);

		keysArray = [
			Preferences.keyBinds.get('note_left').copy(),
			Preferences.keyBinds.get('note_down').copy(),
			Preferences.keyBinds.get('note_up').copy(),
			Preferences.keyBinds.get('note_right').copy()
		];
		...
		strumLine = new FlxSprite(Preferences.data.middleScroll ? STRUM_X_MIDDLESCROLL : STRUM_X, 50).makeGraphic(FlxG.width, 10);
		...
		healthBarBG.visible = !Preferences.data.hideHud;
		healthBarBG.xAdd = -4;
		healthBarBG.yAdd = -4;
		...
		else if (Preferences.data.pauseMusic != 'None')
		{
			precacheList.set(Paths.formatToSongPath(Preferences.data.pauseMusic), 'music');
		}
	...
	public function initRuntimeShader(name:String, ?glslVersion:Int = 120)
	{
		if (!Preferences.data.shaders)
			return false;
		...
		var fragPath:String = Paths.shaderFragment(name);
		var vertPath:String = Paths.shaderVertex(name);
		var found:Bool = OpenFlAssets.exists(fragPath, TEXT) || OpenFlAssets.exists(vertPath, TEXT);
		if (found)
		{
			var frag:String = OpenFlAssets.exists(fragPath, TEXT) ? OpenFlAssets.getText(fragPath) : null;
			var vert:String = OpenFlAssets.exists(vertPath, TEXT) ? OpenFlAssets.getText(vertPath) : null;
	...
	function tankIntro()
	{
		...
		tankman.antialiasing = Preferences.data.globalAntialiasing;
		addBehindDad(tankman);
		cutsceneHandler.push(tankman);

		var tankman2:FlxSprite = new FlxSprite(16, 312);
		tankman2.antialiasing = Preferences.data.globalAntialiasing;
		tankman2.alpha = 0.000001;
		cutsceneHandler.push(tankman2);
		var gfDance:FlxSprite = new FlxSprite(gf.x - 107, gf.y + 140);
		gfDance.antialiasing = Preferences.data.globalAntialiasing;
		cutsceneHandler.push(gfDance);
		var gfCutscene:FlxSprite = new FlxSprite(gf.x - 104, gf.y + 122);
		gfCutscene.antialiasing = Preferences.data.globalAntialiasing;
		cutsceneHandler.push(gfCutscene);
		var picoCutscene:FlxSprite = new FlxSprite(gf.x - 849, gf.y - 264);
		picoCutscene.antialiasing = Preferences.data.globalAntialiasing;
		cutsceneHandler.push(picoCutscene);
		var boyfriendCutscene:FlxSprite = new FlxSprite(boyfriend.x + 5, boyfriend.y + 20);
		boyfriendCutscene.antialiasing = Preferences.data.globalAntialiasing;
	...
	public function startCountdown():Void
	{
		...
		if (true)
		{
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer)
			{
				...
				var introAlts:Array<String> = introAssets.get('default');
				var antialias:Bool = Preferences.data.globalAntialiasing;
				if (isPixelStage)
				{
					introAlts = introAssets.get('pixel');
					antialias = false;
				...
				{
					if (Preferences.data.opponentStrums || note.mustPress)
					{
						note.copyAlpha = false;
						note.alpha = note.multAlpha;
						if (Preferences.data.middleScroll && !note.mustPress)
						{
							note.alpha *= 0.35;
						}
					}
	...
	public dynamic function updateScore(miss:Bool = false)
	{
		...
		if (Preferences.data.scoreZoom && !miss && !cpuControlled)
		{
			...
			accuracyTxt.scale.x = 1.075;
			accuracyTxt.scale.y = 1.075;
			comboRating.scale.x = 1.075;
			comboRating.scale.y = 1.075;
			...
			FlxTween.tween(missesTxt.scale, {x: 1, y: 1}, 0.2);
			FlxTween.tween(accuracyTxt.scale, {x: 1, y: 1}, 0.2);
			FlxTween.tween(comboRating.scale, {x: 1, y: 1}, 0.2);
		}
	}
	...
	private function generateSong(dataPath:String):Void
	{
			...
			for (event in eventsData) // Event Notes
			{
				for (i in 0...event[1].length)
				{
					...
					var subEvent:EventNote = {
						strumTime: newEventNote[0] + Preferences.data.noteOffset,
		...
		for (section in songData.notes)
		{
			for (i in 0...section.sectionNotes.length)
			{
				...
				var chartNoteType:String = Std.isOfType(songNotes[3], String)
					? songNotes[3]
					: ChartingState.noteTypeList[songNotes[3]];
				var isTripleLane:Bool = songData.tripleStrumline == true && songNotes[1] >= 8;
				final leNoteData:ChartNoteData = {
					time: songNotes[0],
					id: Std.int(songNotes[1] % 4),
					sLen: songNotes[2],
					strumLine: gottaHitNote ? 1 : 0,
					isGfNote: isTripleLane,
					tripleLane: isTripleLane,
					type: chartNoteType
				};
		...
		for (note in noteDatas)
		{
			...
			if (note.tripleLane == true)
			{
				swagNote.multAlpha = 0.3;
				swagNote.alpha = 0.3;
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			{
				if (!Preferences.data.opponentStrums)
					targetAlpha = 0;
				else if (Preferences.data.middleScroll)
					targetAlpha = 0.35;
			}

			var babyArrow:StrumNote = new StrumNote(Preferences.data.middleScroll ? STRUM_X_MIDDLESCROLL : STRUM_X, strumLine.y, i, player);
	...
	override public function update(elapsed:Float)
	{
		...
		switch (curStage)
		{
			case 'tank':
				moveTank(elapsed);
			case 'schoolEvil':
				if (!Preferences.data.lowQuality && bgGhouls.animation.curAnim.finished)
				{
					bgGhouls.visible = false;
				}
			case 'philly':
		...
		while (noteTimestamps.length > 0 && now - noteTimestamps[0] > 1000) {
			noteTimestamps.shift();
		}
		nps = noteTimestamps.length;
		if (nps > maxNps) maxNps = nps;
		
		if(ratingName == "N/A")
		{
			accuracyTxt.text = 'Accuracy: -% - ';
			comboRating.text = '[N/A]';
			comboRating.color = FlxColor.GRAY;
		} else {
			accuracyTxt.text = 'Accuracy: ${Highscore.floorDecimal(ratingPercent, 0)}% - ';
			comboRating.text = '[${ratingName}]';
			if (ratingPercent >= 100)
			{
				comboRating.color = FlxColor.fromRGB(180, 120, 255);
			}
			else if (ratingPercent >= 70)
			{
				comboRating.color = FlxColor.fromRGB(90, 240, 255);
			}
			else if (ratingPercent >= 40)
			{
				comboRating.color = FlxColor.fromRGB(255, 230, 120);
			}
			else
			{
				comboRating.color = FlxColor.RED;
			}
		}
		...
		else
		{
			if (!paused)
			{
				...
				if (updateTime)
				{
					var curTime:Float = Conductor.songPosition - Preferences.data.noteOffset;
					if (curTime < 0)
						curTime = 0;
					songPercent = (curTime / songLength);

					var songCalc:Float = (songLength - curTime);
					if (Preferences.data.timeBarType == 'Time Elapsed')
						songCalc = curTime;
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String)
	{
		...
		switch (eventName)
		{
			...
			case 'Trigger BG Ghouls':
				if (curStage == 'schoolEvil' && !Preferences.data.lowQuality)
				{
					bgGhouls.dance(true);
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		if (!Preferences.data.comboStacking && comboGroup.members.length > 1)
		{
		...
		rating.visible = (!Preferences.data.hideHud && showRating);
		...
		if(cpuControlled) {
			rating.visible = false;
					...
					var msTiming = Highscore.floorDecimal(noteDiff, 3);
			...
			{
				case 'shit' | 'bad':
					this.currentTimingShown.color = FlxColor.RED;
				case 'good':
					this.currentTimingShown.color = FlxColor.GREEN;
				case 'sick':
					this.currentTimingShown.color = FlxColor.CYAN;
			}

						this.currentTimingShown.borderStyle = OUTLINE;
			this.currentTimingShown.borderSize = 1;
			this.currentTimingShown.borderColor = FlxColor.BLACK;
			this.currentTimingShown.text = msTiming + "ms";
			this.currentTimingShown.size = 20;

						if (this.currentTimingShown.alpha != 1)
				this.currentTimingShown.alpha = 1;

			comboGroup.add(this.currentTimingShown);

		this.timingFadeTimer = 0;
		...
		comboSpr.visible = (!Preferences.data.hideHud && showCombo);
		...
		if (!PlayState.isPixelStage)
		{
			...
			rating.antialiasing = Preferences.data.globalAntialiasing;
			comboSpr.setGraphicSize(Std.int(comboSpr.width * 0.7));
			comboSpr.antialiasing = Preferences.data.globalAntialiasing;
		}
		else
		{
		...
		for (i in seperatedScore)
		{
			...
			numScore.x += Preferences.data.comboOffset[2];
			numScore.y -= Preferences.data.comboOffset[3];

			if (!PlayState.isPixelStage)
			{
				numScore.antialiasing = Preferences.data.globalAntialiasing;
			...
			numScore.visible = !Preferences.data.hideHud;
	...
	private function onKeyPress(event:KeyboardEvent):Void
	{
		...
		var shouldMiss:Bool = !Preferences.data.ghostTapping;
...
RankedStats.save();
	...
	function gfNoteHit(note:Note):Void
	{
		if (gf != null && !note.noAnimation)
		{
			if (note.noteType == 'Dad Sing')
			{
	...
	function goodNoteHit(note:Note):Void
	{
		...
		if(!isSus) {
			totalNotes += 1;
			noteTimestamps.push(Conductor.songPosition);
		}

		note.wasGoodHit = true;
		if (Preferences.data.hitsoundVolume > 0 && !note.hitsoundDisabled)
		...
		if (!note.noAnimation)
		{
			...
			if (note.gfNote)
			{
				if (gf != null && note.noteType != 'Dad Sing')
				{
	...
	{
		if (Preferences.data.noteSplashes && note != null)
		{
	...
	public function spawnNoteSplash(x:Float, y:Float, data:Int, ?note:Note = null)
	{
		...
		var sat:Float = 0;
		var brt:Float = 0;
		if (data > -1 && data < Preferences.data.arrowHSV.length)
		{
			hue = Preferences.data.arrowHSV[data][0] / 360;
			sat = Preferences.data.arrowHSV[data][1] / 100;
			brt = Preferences.data.arrowHSV[data][2] / 100;
			if (note != null)
			{
	...
	{
		if (!Preferences.data.lowQuality && Preferences.data.violence && curStage == 'limo')
		{
		...
		for (name in achievesToCheck)
		{
			...
			if (name != WeekData.getWeekFileName() + '_nomiss') // common achievements
			{
				switch (name)
				{
					...
					case 'toastie':
						unlock = (!Preferences.data.shaders && Preferences.data.lowQuality && !Preferences.data.globalAntialiasing);
