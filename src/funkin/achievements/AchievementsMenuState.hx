...
class AchievementsMenuState extends MusicBeatState
{
	...
	private var progressBarBG:AttachedSprite;
	...
	override function create()
	{
		...
		progressBarBG.color = FlxColor.BLACK;
		...
		progressTxt = new FlxText(50, progressBarBG.y - 6, FlxG.width - 100, "", 32);
	...
	public static inline function sortByID(Obj1:Dynamic, Obj2:Dynamic):Int
	...
	override function update(elapsed:Float) {
		if(!goingBack && options.length > 1)
		{
			...
			if(add != 0)
			{
				final oldRow:Int = Math.floor(curSelected / MAX_PER_ROW);
				final rowSize:Int = Std.int(Math.min(MAX_PER_ROW, options.length - oldRow * MAX_PER_ROW));
		...
		FlxG.camera.followLerp = flixel.math.FlxMath.bound(elapsed * 9 / (FlxG.updateFramerate / 60), 0, 1);
		...
		if(controls.ACCEPT)
		{
			if(onYes)
			{
				...
				if(state.progressBar.visible)
				{
					...
					state.barTween = FlxTween.tween(state.progressBar, {value: 0}, 0.5, {ease: FlxEase.quadOut});
				}
				Achievements.save();
				FlxG.save.flush();
